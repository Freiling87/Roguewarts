...
namespace Roguewarts.Abilities.Magic
{
	public class TelemanticBlink : CustomAbility, IAbilityRechargeable, IAbilityChargeable
	{
		...
		public void OnRecharging(AbilityRechargingArgs e)
		{
			...
			if (tbNetCharge == 0 && Item.invItemCount < AbilityManager.CalcMaxMana(Item.agent) && Item.agent.statusEffects.CanRecharge())
			{
				Item.invItemCount = Math.Min(AbilityManager.CalcMaxMana(Item.agent), Item.invItemCount + MSA_TB_RollRechargeRate(Item.agent));

				if (Item.invItemCount == AbilityManager.CalcMaxMana(Item.agent))
		...
		public static void MSA_TB_DialogueCantDo(Agent agent)
		{
			...
			HeaderTools.SayDialogue(agent, HeaderTools.RandomFromList(dialogue), vNameType.Dialogue);
		}
		...
		public static Vector2 MSA_TB_RollDestination(Agent agent, float minError, float maxError)
		{
			...
			for (int i = 0; i < 50; i++)
			{
				...
				targetPosition = AbilityManager.MouseIngamePosition() + (distance * Random.insideUnitCircle.normalized);
