...
namespace Roguewarts.Abilities.Magic
{
	public class ChronomanticDilation : CustomAbility, IAbilityRechargeable
	{
		...
		public override void OnPressed()
		{
			...
			else if (MSA_CD_RollMiscast(Item.agent, (float)((AbilityManager.CalcMaxMana(Item.agent) - Item.invItemCount) / 100f)))
		...
		public void OnRecharging(AbilityRechargingArgs e)
		{
			...
			else if (Item.invItemCount < AbilityManager.CalcMaxMana(Item.agent) && Item.agent.statusEffects.CanRecharge())
			{
				Item.invItemCount += Math.Min(AbilityManager.CalcMaxMana(Item.agent) - Item.invItemCount, 5);

				if (Item.invItemCount == AbilityManager.CalcMaxMana(Item.agent) && !MSA_CD_IsMiscast(Item.agent))
